...
class RawFirFragmentForLazyBodiesBuilder private constructor(
    ...
    private val original: FirDeclaration
    ...
    companion object {
        fun build(
            ...
            original: FirDeclaration
        ): FirDeclaration {
            val builder = RawFirFragmentForLazyBodiesBuilder(session, baseScopeProvider, declaration, original)
            builder.context.packageFqName = declaration.containingKtFile.packageFqName
            return builder.moveNext(designation.iterator())
        }
    }

    private tailrec fun moveNext(iterator: Iterator<FirDeclaration>): FirDeclaration {
        if (!iterator.hasNext()) {
            return (declaration.accept(Visitor(), Unit) as FirDeclaration).also {
                it.copyContainingClassAttrFrom(original)
            }
        }
        ...
        require(classOrObject is KtClassOrObject)
